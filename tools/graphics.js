...
.ph{position:absolute;border-radius:42px;overflow:hidden;background:${K.ink};border:9px solid ${K.ink};box-shadow:0 60px 100px -40px rgba(34,15,33,.8),0 0 0 1px rgba(255,255,255,.18)}
...
.in{position:absolute;border-radius:16px;overflow:hidden;background:#fff;border:6px solid #fff;box-shadow:0 50px 90px -40px rgba(34,15,33,.8)}
...
function biodynamic(b) {
  ...
  <div style="position:absolute;inset:88px 22px 22px;border-radius:18px;background:#F3E4F4;overflow:hidden"><div style="position:absolute;inset:0;background:${lattice(K.lapis, .16, 96)}"></div>
  ${dots.map(([x, y, n, r]) => `<div style="position:absolute;left:${x - r}px;top:${y - r}px;width:${r * 2}px;height:${r * 2}px;border-radius:50%;background:rgba(178,65,175,.18)"></div><div style="position:absolute;left:${x - 24}px;top:${y - 24}px;width:48px;height:48px;border-radius:50%;background:${K.lapis};border:4px solid #fff;display:grid;place-items:center;font-weight:800;font-size:17px;color:#fff">${n}</div>`).join('')}
  <div style="position:absolute;right:22px;bottom:22px;background:#fff;border-radius:16px;padding:16px 20px;display:flex;gap:14px;align-items:center;box-shadow:0 20px 40px -20px rgba(34,15,33,.4)"><i style="width:44px;height:44px;border-radius:50%;background:linear-gradient(135deg,${K.tint},${K.lapis})"></i><div style="font-weight:700;font-size:17px">Certified practitioner<div style="font-weight:500;font-size:14px;color:#5B615C">City, country, profile page</div></div></div></div></div>
  ${[['173', 'practitioner profiles, each one a record with photo, city and coordinates', 120], ['Leaflet', 'map drawn from those same records, listing alongside', 392], ['39 to 92', 'mobile PageSpeed at hand over. Desktop went from 83 to 98', 664]].map(([n, t, y]) => `<div style="position:absolute;left:990px;top:${y}px;width:320px;height:246px;border-radius:26px;background:#fff;padding:30px 28px;box-shadow:0 50px 90px -50px rgba(34,15,33,.7)"><div class="d" style="font-size:${n.length > 5 ? 50 : 62}px;color:${K.lapis}">${n}</div><div style="margin-top:14px;font-size:18px;line-height:1.4;font-weight:500;color:#3F4641">${t}</div></div>`).join('')}`;
}
...
async function mockups() {
  ...
  for (const slug of C.order) {
    ...
    if (!has(shot)) { // no screenshot exists: a designed name tile
      await shoot(stage(1400, 1050, BACKDROPS[1], `<div style="position:absolute;inset:0;display:grid;place-content:center;text-align:center;gap:26px;color:#fff"><div class="d" style="font-size:150px">${p.name.replace(' Consultant', '<br>Consultant')}</div><div style="font-size:34px;font-weight:500;color:#E9C6EA">${p.industry}, ${p.location}</div></div>`), 1400, 1050, out(slug + '-card'));
...
async function og() {
  ...
  const items = [
    ...
    ['demos', 'Watch WordPress problems get fixed, and a design get built.', 'Live demos', card('desert-safari-dubai')],
...
<i class="glow" style="width:620px;height:620px;left:-220px;top:-300px;background:rgba(243,88,238,.8)"></i><i class="glow" style="width:420px;height:420px;right:-120px;bottom:-220px;background:rgba(117,255,66,.5)"></i><i class="glow" style="width:44%;height:62%;left:40%;top:16%;background:rgba(215,129,9,.36)"></i><i class="glow" style="width:300px;height:300px;right:300px;top:-200px;background:rgba(248,163,246,.4)"></i>
<div class="pat" style="background:${lattice('#E9C6EA', .2, 120)};-webkit-mask-image:linear-gradient(90deg,#000,transparent 70%)"></div>
...
<div style="position:absolute;left:64px;top:168px;width:540px"><div style="font-size:22px;font-weight:700;color:#F8A3F6;margin-bottom:18px">${kicker}</div><div class="d" style="font-size:${size}px">${title}</div></div>
<div style="position:absolute;left:64px;bottom:52px;display:flex;gap:14px;align-items:center;font-size:21px;font-weight:700"><span style="background:${K.gold};color:${K.ink};border-radius:999px;padding:12px 24px">Free first look</span><span style="color:#D9BBDA;font-weight:500">WordPress, WooCommerce, Elementor Pro</span></div>
