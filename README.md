...
10. **Seven scroll sequences, one source.** Each lives once in `src/fragments/demo-<name>.html` and is pulled in with `{{demo:name}}`. The homepage shows three (checkout, site down, speed). `/demos/` shows all seven, adding broken mobile layout, enquiry emails that never arrive, content the team cannot edit, and one build: a Figma or PSD design turned into a WordPress page (a design layer wiped away by a dragged handle to show the built page beneath, then resized through 1440, 768 and 390 using a container query). The page heading counts repairs and builds from the `DEMOS` list, so it cannot go stale. There is a jump index at the top (`DEMOS` list and `demosPage()` in `build.js`). Every sequence is a `.repair-pin` with a `data-demo` name and a timeline of the same name in `src/app.js`; `data-len` sets how much scrolling it takes. Shared parts: `scanAndNotes` (scan line and the four notes), `flags` (any element with `data-flag` gets a callout pin, red for a fault or blue with `data-flag-tone="spec"` for a measurement, that pops as the scan reaches it, turns green at the fix and leaves), and `pointer`/`go`/`click` (an on screen cursor that does the clicking). The markup holds the fixed state and the script sets the broken state, so reduced motion and no script both show a sensible result. All are labelled as examples. The only real number in them is the 39 to 92 score. To add another: write the fragment, add a timeline, add a line to `DEMOS`.
